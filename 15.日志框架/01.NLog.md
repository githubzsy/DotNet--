# NLog
1. 官网: https://nlog-project.org/download/
2. 使用教程: https://github.com/NLog/NLog/wiki/Getting-started-with-ASP.NET-Core-2, https://www.cnblogs.com/jbjw/articles/13925060.html
3. 引入包``NLog.AspNetCore``
4. 创建``nlog.config``配置文件
    ```XML
...
6. 配置``Program``
    ...
    ```
7. 也可以在``appsettings.json``中配置最低日志级别, 将会覆盖代码中的设置
    ```json
    {
        "Logging": {
            "LogLevel": {
...
8. 使用
    ...
    ```
9. ``nlog.config``配置说明: https://github.com/NLog/NLog/wiki/Configuration-file#rules